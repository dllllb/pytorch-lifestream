...
def test_embeds_quantiles():
    ...
    q_embeds, bins = embeds_quantiles(array, q_count=4)
    assert np.array_equal(q_embeds, out)

    q_embeds = embeds_quantiles(array, bins=bins)
    assert np.array_equal(q_embeds, out)
...
def test_pca_reduction():
    ...
    reduced_array, pca_list = pca_reduction(array, sub_dim=4, n_components=1)
    assert np.allclose(reduced_array, out, rtol=1e-07)

    reduced_array = pca_reduction(array, sub_dim=4, pca_list=pca_list)
    assert np.allclose(reduced_array, out, rtol=1e-07)
