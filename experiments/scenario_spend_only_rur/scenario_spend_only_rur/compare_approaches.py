...
from scenario_spend_only_rur.features import load_features, load_scores
...
def get_scores(args):
    ...
    for fold_n, (valid_fold, test_fold) in enumerate(zip(valid_scores, test_scores)):
        ...
        result.append({
            'name': name,
            'fold_n': fold_n,
            'oof_accuracy': valid_metr,#(valid_fold['pred'] == valid_fold[COL_TARGET]).mean(),
            'test_accuracy': test_metr,#(test_fold['pred'] == test_fold[COL_TARGET]).mean(),
        })
