...
from scenario_protein.const import (
    COL_ID, COL_TARGET, DEFAULT_DATA_PATH, DEFAULT_RESULT_FILE, DATASET_FILE, TEST_IDS_FILE,
)
from scenario_protein.features import load_features, load_scores
...
def filter_target(df, col_target_name):
    df[col_target_name] = (df[col_target_name] > 60).astype(int)
    return df


def top_k_precision(y_true, y_predict, k=0.05):
    if type(y_predict) is pd.Series:
        y_predict = y_predict.values
    if type(y_true) is pd.Series:
        y_true = y_true.values

    n = int(len(y_predict) * k)
    ix = np.argsort(y_predict)
    top_ix = ix[-n:]
    return y_true[top_ix].mean()
...
def main(conf):
    ...
    if len(approaches_to_train) > 0:
        ...
        args_list = [sct.KWParamsTrainAndScore(
            ...
            scorer_name='top_5pp_precision',
            scorer=make_scorer(partial(top_k_precision, k=0.05), needs_proba=True),
        ...
        df_results = pd.DataFrame(results).set_index('name')[['oof_top_5pp_precision', 'test_top_5pp_precision']]
    ...
    df_results = sct.group_stat_results(df_results, 'name', ['oof_top_5pp_precision', 'test_top_5pp_precision'])
