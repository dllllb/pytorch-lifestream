...
def load_data_csv(file_name_in, id_col, trx_type_col, amount_col):
    df = pd.read_csv(file_name_in)
    grouped = df.groupby(id_col)
    res = {}
    res['tr_type'] = grouped[trx_type_col].apply(list)
    res['amount'] = grouped[amount_col].apply(list)
...
def get_distributions(np_data, tr_amounts_col, tr_types_col=None,
    ...
    if len(set_top_neg_types) == 0:
        sums_of_negative_target = neg_distribution = None
    if len(set_top_pos_types) == 0:
        sums_of_positive_target = pos_distribution = None
...
def write_target(filename_out, ids, sums_of_negative_target, sums_of_positive_target,
                 neg_distribution, pos_distribution, id_col):
    if neg_distribution and pos_distribution:
        data_to_write = list(zip(ids, zip(sums_of_negative_target,
                                          neg_distribution,
                                          sums_of_positive_target,
                                          pos_distribution)))
    elif neg_distribution:
        data_to_write = list(zip(ids, zip(sums_of_negative_target,
                                          neg_distribution)))
    elif pos_distribution:
        data_to_write = list(zip(ids, zip(sums_of_positive_target,
                                          pos_distribution)))       

    output_data_df = pd.DataFrame(data=data_to_write, columns=[id_col, 'distribution'])
    output_data_df.to_csv(filename_out, index=False)
    

def create_new_targets_on_gender_train_csv(file_name_in, filename_out, id_col, trx_type_col,
                                           amount_col, TR_AMOUNTS_COL=2, TR_TYPES_COL=1,
                                           top_THR=5, take_first_fraction=0.5):
    '''
    ...
    # load data to numpy
    np_data, columns = load_data_csv(file_name_in, id_col, trx_type_col, amount_col)
    ids = np_data[:, columns.index(id_col)]
    ...
    with open('data/negative.pickle', 'wb') as neg:
        pickle.dump(negative_items, neg)
    with open('data/positive.pickle', 'wb') as pos:
...
if __name__ == '__main__':
    create_new_targets_on_gender_train_csv('data/transactions_train.csv',
                                           'data/age_train_distribution_target.csv',
                                           'client_id', 'small_group', 'amount_rur')
