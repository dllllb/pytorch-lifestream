...
for labeled_amount in [337]: #, 675, 1350, 2700, 5400, 10800]:
    ...
    bash_command = [
        ...
        f'params.labeled_amount={labeled_amount}',
        f'output.test.path="../data/age-pred/pseudo_labeling_{labeled_amount}"/test',
        f'output.valid.path="../data/age-pred/pseudo_labeling_{labeled_amount}"/valid',
        '--conf conf/age_pred_target_dataset.hocon conf/age_pred_pseudolabel_params_train.json'
    ]
    bash_command = ' '.join(bash_command)
    ...
    # compare approaches (fit target vs. fit tunning vs. pseudo-labeling vs. ml embeddings vs. baseline(GBDT))
    bash_command = [
