...
def fold_fit_test(conf, fold_id):
    ...
    valid_metrics = {name:trainer.logged_metrics[f"val_{name}"].item() for name, _ in model.valid_metrics.items()}
    trainer.test(model=model, dataloaders=dm.test_dataloader(), ckpt_path="best", verbose=False)
    test_metrics =  {name:trainer.logged_metrics[f"test_{name}"].item() for name, _ in model.test_metrics.items()}
