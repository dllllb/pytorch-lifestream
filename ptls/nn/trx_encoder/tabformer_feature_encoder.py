...
class TabFormerFeatureEncoder(nn.Module):
    ...
    def __init__(self, n_cols: int,
        ...
        out_hidden = out_hidden if out_hidden else emb_dim * (n_cols + 1)
        self.token_sep = torch.nn.Parameter(torch.randn(1, 1, emb_dim), requires_grad=True)
        ...
        self.lin_proj = nn.Linear(emb_dim * (n_cols + 1), out_hidden)
    ...
    def forward(self, input_embeds):
        ...
        input_embeds = torch.cat((input_embeds, self.token_sep.repeat(1, input_embeds.shape[1], 1)), dim=0)
