...
def train_multicoles_sml_model_folder(fold_i, exp_name, dataf, trx_conf, input_size, hsize,
                                      clf_hsize, path_to_logs, path_to_chkp, embed_coef, gpu_n, max_epoch, debug):
    datamodule = dataf(fold_i)
    module = get_multicoles_sml_module(trx_conf, input_size, embed_coef, hsize, clf_hsize)
    writer = TensorBoardLogger(save_dir=path_to_logs, name='sml_fold_'+str(fold_i))
    trainer = pl.Trainer(gpus=[gpu_n],
                         max_epochs=max_epoch,
                         enable_checkpointing=False,
                         enable_progress_bar=False,
                         ...
                         fast_dev_run=debug)
    logger_version = trainer.logger.version
    tb_name = exp_name + '/' + 'sml_fold_' + str(fold_i) + '/' + str(logger_version)
    trainer.fit(module, datamodule)
    name = '_'.join(['sml', str(fold_i)])
    name = add_next_ind(path_to_chkp, name) + '.pth'
    model_save_path = os.path.join(path_to_chkp, name)
    torch.save(module.seq_encoder.state_dict(), model_save_path)
    return fold_i, tb_name, model_save_path
...
def train_multicoles_sml_model(path_to_logs, path_to_chkp, fold_i, embed_coef, gpu_n,
                           conf_path='./config.hocon', debug=False):
    conf = ConfigFactory.parse_file(conf_path)
    dataset = conf.get('dataset')
    assert dataset in ['synthetic', 'age_pred', 'alpha_battle'], 'invalid dataset'
    ...
    paths_to_models = train_multicoles_sml_model_folder(fold_i, exp_name, dataf, trx_conf,
                                                        input_size, hsize, clf_hsize, path_to_logs, path_to_chkp,
                                                        embed_coef, gpu_n, max_epoch, debug)
    return paths_to_models
